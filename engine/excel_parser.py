...
class ExcelParser:
    ...
    def ler_planilha_carol(caminho):
        """
        Lê a planilha de Controle de Empregados (.xls).
        ...
        logging.info(f"[DP] Lendo Planilha da Carol (.xls): {caminho}")
        try:
            ...
            logging.error(f"[DP] Erro ao processar planilha da Carol: {e}")
            return None
