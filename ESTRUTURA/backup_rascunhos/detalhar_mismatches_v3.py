...
def main():
    # 1. Carregar Fonte (Sem data_only=True para ser mais rápido)
    ...
    for r in range(10, ws_dst.max_row + 1):
        ...
        if cod not in src_map:
            list_cases.append(f"{cod:<6} | {str(nome)[:40]:<40} | Não consta na planilha Contábil")
            count_missing += 1
        elif src_map[cod] != cnpj_master:
            list_cases.append(f"{cod:<6} | {str(nome)[:40]:<40} | CNPJ Divergente (Fonte: {src_map[cod]})")
            count_divergent += 1
