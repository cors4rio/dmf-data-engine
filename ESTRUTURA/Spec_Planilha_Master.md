...
|---|---|---|---|
| Escrita Fiscal | O | Banco Domínio (`GELOGUSER`) + adicional 70% | [Spec_Produtividade_Fiscal.md](./Spec_Produtividade_Fiscal.md) |
...
**O que vai aqui:** Tempo total (em segundos convertidos) que os colaboradores ficaram no módulo Fiscal do Domínio para aquele cliente no mês, **com adicional de 70%**.

**Fórmula:**
```
segundos_brutos = SUM(DATEDIFF(second, ...)) por cliente na GELOGUSER (sist_log=5)
tempo_final = segundos_brutos × 1.70
...
└─────────────────────────────────────────────────────────────┘
  ...
  │     Aplicar adicional 70% antes de gravar            │
  │     Match: codi_emp → CNPJ                           │
  └───────────────────┬──────────────────────────────────┘
                      ▼
  ┌──────────────────────────────────────────────────────┐
...
| Aplicar o adicional de 70% antes de salvar os segundos brutos | Salvar bruto no dict, aplicar 70% apenas na conversão para Excel |
...
|---|---|---|
| [Spec_Produtividade_Fiscal.md](./Spec_Produtividade_Fiscal.md) | O e N | O adicional de 70% é aplicado ANTES de gravar em O. N recebe o valor de O do mês anterior **já com** o adicional. |
