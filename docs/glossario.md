...
**Carol (Planilha de Controle de Empregados)**
Planilha `.xls` produzida mensalmente pela responsável do DP (Carol) com o número de empregados ativos por empresa. Usada como fonte de dados para o módulo DP. Não é extraída automaticamente do Domínio — é um arquivo manual entregue pelo setor.

**Central DMF**
Componente de plataforma do sistema: interface gráfica (PyWebView), autenticação, plugin system e orquestração de módulos. Roda em Python 64-bit (quando as dependências residuais de `engine/` forem removidas). Corresponde ao diretório `dmf_engine/`.
...
**Domínio (ERP)**
Sistema ERP Domínio Sistemas (da Benner), usado pelo escritório DMF para registro de horas, folha de pagamento e contabilidade dos clientes. O banco de dados é Sybase SQL Anywhere, acessado via ODBC 32-bit.
...
**ODBC**
Open Database Connectivity — interface padrão para conexão com bancos de dados. A Automação de Horas usa ODBC 32-bit para conectar ao Sybase SQL Anywhere (ERP Domínio). Requer driver SQL Anywhere instalado e DSN `Contabil` configurado via `odbcad32.exe`.
...
**SSO por Token**
Single Sign-On implementado via arquivo temporário em `temp/`. A Central gera um token JSON com a sessão do usuário e o processo filho (Automação de Horas) o consome antes que expire (30 segundos). Ver [design-patterns.md — SSO por Token](design-patterns.md#4-sso-por-token).
